...
class QXFBenchPlotter:
    ...
    def __init__(self, workflow_directory: str, workflow_deployment_id: str, run_id: str, format: str, artifacts_filepath: str):
        ...
        self.__artifacts_filepath = artifacts_filepath
        ...
        if not os.path.exists(self.__artifacts_filepath):
            raise FileNotFoundError(f"artifact.json Not Found")
    ...
    def __init__dag( self,user_config_path):
        ...
        for node in __dag_config_data["Nodes"]:
            nodeID = node["NodeId"]
            # TODO - add a better way to add node NodeNames to the DAG
    ...
    def __print_stats_stagewise(self, timings_dict):
        ...
        for f_id in sorted(func_timings):
            logger.info(f"{self.__xfaas_dag.nodes[f_id]['NodeName']} \
        ...
        for e_id in sorted(edge_timings):
            ...
            u = split[0]
            v = split[1]
            logger.info(f"{self.__xfaas_dag.nodes[u]['NodeName']} -> {self.__xfaas_dag.nodes[v]['NodeName']} \
        ...
        def get_labels(label_ids):
            ...
            for label in label_ids:
                if "-" in label:
                    ...
                    node1 = split[0]
                    node2 = split[1]
                    labels.append(f"{self.__xfaas_dag.nodes[node1]['NodeName']}-{self.__xfaas_dag.nodes[node2]['NodeName']}")
                else:
                    labels.append(self.__xfaas_dag.nodes[label]['NodeName'])
            return labels
