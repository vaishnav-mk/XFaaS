...
def check(obj):
    ...
    if body["Poll"] == "Yes":
        logging.info("We are recalling and current obj:"+ json.dumps(body))
        return True
    else:
        logging.info("We are exiting loop and current obj:"+ json.dumps(body))
        return False
...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    apdb = yield context.call_activity("InvokeAndCircuitSplit", serwoObject)
    hwhq = yield context.call_activity("SubmitCircuits", apdb)
    pxcc = yield context.call_activity("WaitFn", hwhq)
    kwoy = yield context.call_activity("Poll", pxcc)   
    kwoy = insert_end_stats_in_metadata(kwoy)
    kbau = yield context.call_activity("CollectLogs", kwoy)
    return kbau
